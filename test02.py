...
def prep_data(video_x=-1):
    ...
    for j in range(nextFrameModel.number_of_batches):
        ...
        for i in range(nextFrameModel.number_of_frames):
            big_array.append(np.array(Image.open("video/frames"+str(video_x).zfill(2)+"/frame-"+str(frame_x[j]+i).zfill(4)+".bmp")))
        big_array02.append(big_array)
        big_array03.append([np.array(Image.open("video/frames"+str(video_x).zfill(2)+"/frame-"+str(frame_x[j]+nextFrameModel.number_of_frames).zfill(4)+".bmp"))])
    ...
    #big_array06.append([np.array(Image.open("video/frames"+str(video_x).zfill(2)+"/frame-"+str(frame_x[nextFrameModel.number_of_batches] +
...
def retrain(epochs=5):
    ...
    model.fit([dataR, dataG, dataB], outFrames, epochs=epochs, batch_size=25)
